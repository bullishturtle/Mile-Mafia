...
function TimelineItem({ entry, index, scrollProgress }: TimelineItemProps) {
  ...
  return (
    <motion.div ref={itemRef} style={{ opacity, scale }} className="relative mb-20 md:mb-32">
      {/* Timeline Dot */}
      <div className="absolute left-1/2 top-1/2 w-4 h-4 bg-primary rounded-full transform -translate-x-1/2 -translate-y-1/2 z-10 hidden md:block" />
          ...
          >
            <div className="sticky top-20">
              <div className="relative overflow-hidden rounded-2xl aspect-[3/4] bg-card border border-border shadow-lg">
                ...
                />
                <div className="absolute inset-0 bg-black/50" /> {/* Increased overlay for luxury theme */}
              </div>
            </div>
          </div>
          ...
          >
            <div className="sticky top-32">
              <motion.div
                ...
                className="space-y-6"
              >
                <h3 className="text-3xl md:text-4xl lg:text-5xl font-black tracking-wide text-foreground">
                  {entry.title}
                </h3>

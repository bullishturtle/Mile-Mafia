...
const Word = ({ children, progress, range }: WordType) => {
  ...
  return (
    <span className="relative me-2 mt-2">
      <span style={{ position: "absolute", opacity: 0.1, color: "hsl(var(--foreground))" }}>{children}</span>
      <motion.span
        style={{
          transition: "all .5s",
          opacity: opacity,
          textShadow: "0 1px 2px rgba(0, 0, 0, 0.5)", // Darker shadow for dark theme
          color: "hsl(var(--foreground))", // Ensure text is foreground color
        }}
      >
        {children}
      </motion.span>
  ...
  return (
    <span>
      <span
        ...
        style={{ color: "hsl(var(--foreground))" }} // Adjusted color
      >
        {children}
      </span>
      <motion.span
